...
<template>
  <div
    class="absolute top-3 left-15 w-96 min-h-40 bg-white border-2 border-sky-800 shadow rounded-xl p-4 z-20 flex flex-col">
    ...
    <div v-else class="grow flex flex-col justify-between">
      <div class="flex gap-2 items-center mb-2 font-medium leading-relaxed">
        <LoaderCircle v-if="address.displayName === PRE_ADDRESS_DISPLAY_NAME" class="size-6 animate-spin" />
        <p>{{ address.displayName }}</p>
      </div>
      <div class="flex gap-2 items-center">
        <MapPin class="inline size-5" />
        <p>{{ formatLatLng(address) }}</p>
      </div>
    </div>
  </div>
</template>